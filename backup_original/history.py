...
def history():
    ...
    try:
        ...
        totalAprob = float(totales[0]) if totales[0] is not None else 0.0
        totalRecha = float(totales[1]) if totales[1] is not None else 0.0
...
def descargar_historico_excel():
    ...
    try:
        ...
        cursor.execute('''
            SELECT
                tipoNom,
                SUM(cantAprob) as total_aprobados,
                SUM(cantRecha) as total_rechazados,
